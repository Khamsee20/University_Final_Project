                ...
                <div class="row gx-4">
                    ...
                    <div class="col-lg-4 col-md-6 my-sm-auto ms-sm-auto me-sm-0 mx-auto mt-3">
                        <div class="nav-wrapper position-relative end-0">
                            <form action="{{ route('member_register-room.search') }}" method="GET"
                ...
                <div class="card mb-4">
                    <div class="card-header pb-3">
                        ...
                        <a class="btn btn-primary float-end px-6 mx-4"
                            href="{{ route('member_register-room.create') }}">ເພີ່ມຫ້ອງແຖວໃໝ່</a>
                    </div>
                    <div class="card-body px-3 pt-1 pb-2 ">
                        <div class="table-responsive p-0">
                                ...
                                <tbody>
                                    @foreach ($roomget as $item)
                                        {{-- @if ($item->member_id ==
                                            implode(
                                                ',',
                                                Auth::user()->member->pluck('id')->toArray(),
                                            ))  --}}
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex px-3 py-2">
                                                        <div class="d-flex flex-column justify-content-center">
                                                            @if ($item->status == 'ອອນລາຍ')
                                                            <h6 style="color:green" class="mb-0 text-md">{{ $item->status }}</h6>
                                                            @else
                                                            <h6 style="color: red" class="mb-0 text-md">{{ $item->status }}</h6>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="align-right">
                                                    <form action="{{ route('member_register-room.delete', $item->id) }}"
                                                        method="POST">
                                                        <a href="{{ route('member_register-room_details.edit', $item->id) }}"
                                                            class="btn btn-success btn-sm" data-toggle="tooltip"
                                                            data-original-title="Edit roomtype">
                                                            ລາຍລະອ່ຽດ
                                                        </a>

                                                        <a href="{{ route('member_register-room.edit', $item->id) }}"
                                                            class="btn btn-success btn-sm" data-toggle="tooltip"
                                                            data-original-title="Edit roomtype">
                                                            ແກ້ໄຂ
                                                        </a>
                            ...
                            {{-- {{ implode(',',Auth::user()->member->pluck('id')->toArray()) }} --}}
                            {{-- {{ Auth::user()->member }} --}}
